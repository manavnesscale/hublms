...
from hublms.hublms.doctype.hublms_question.hublms_question import validate_correct_answers
...
class HublmsQuiz(Document):
	...
	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "Hublms Quiz")
...
def quiz_summary(quiz,course, results):
	...
	for result in results:
		...
		result["question"] = frappe.db.get_value(
			"Hublms Question", question_details.question, "question"
		)
...
def save_quiz(
	...
	if quiz:
		frappe.db.set_value("Hublms Quiz", quiz, values)
		update_questions(quiz, questions)
		return quiz
	else:
		doc = frappe.new_doc("Hublms Quiz")
		doc.update(values)
		doc.save()
...
def update_questions(quiz, questions):
	...
	frappe.db.set_value("Hublms Quiz", quiz, "total_marks", set_total_marks(quiz, questions))
...
def delete_questions(quiz, questions):
	...
	for question in existing_questions:
		if question not in current_questions:
			frappe.db.delete("Hublms Quiz Question", question)
...
def add_questions(quiz, questions):
	for index, question in enumerate(questions):
		...
		if question.question_name:
			doc = frappe.get_doc("Hublms Quiz Question", question.question_name)
		else:
			doc = frappe.new_doc("Hublms Quiz Question")
			doc.update(
				{
					"parent": quiz,
					"parenttype": "Hublms Quiz",
...
def save_question(quiz, values, index):
	...
	if values.get("name"):
		doc = frappe.get_doc("Hublms Question", values.get("name"))
	else:
...
def get_question_details(question):
	if frappe.db.exists("Hublms Quiz Question", question):
		...
		return frappe.db.get_value("Hublms Quiz Question", question, fields, as_dict=1)
	return
...
def get_user_quizzes():
	...
	return frappe.get_all("Hublms Quiz", filters=filters, fields=["name", "title"])
